...
# 初始化 SocketIO (使用 threading 模式，兼容 Windows 环境)
socketio = SocketIO(
    app,
...
def classify_intent(text: str) -> str:
    """
    智能意图识别（4层架构）
    输入层(text) → 编码层(keyword matching) → 分类层(priority scoring) → 响应层(intent label)
    对应论文第3章图3-1的意图识别与路由流程
    返回意图类别常量字符串
    """
    try:
        text = (text or "").strip()
        ...
        for keyword, intent in INTENT_KEYWORD_MAP:
            if keyword in text:
        ...
        # 优先匹配转人工和投诉（高优先级意图）
        high_priority = [INTENT_TRANSFER_HUMAN, INTENT_COMPLAINT]
        for hp in high_priority:
            if hp in matched_intents:
                return hp

        # 否则返回匹配次数最多的意图
        best_intent = max(matched_intents, key=lambda k: matched_intents[k])  # type: ignore
        return best_intent
    except Exception as e:
